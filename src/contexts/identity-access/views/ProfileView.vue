...
.profile-overview h2 { margin: 3px 0 4px; color: var(--cocoa-950); font: 800 28px/1.15 var(--font-display); }.profile-overview p:last-child { margin: 0; color: var(--cocoa-700); }
.profile-section-heading { display: flex; align-items: flex-start; gap: 14px; margin-bottom: 22px; }.profile-section-heading h2 { margin: 0; color: var(--cocoa-950); font: 800 20px/1.2 var(--font-display); }.profile-section-heading h2 small { color: var(--cocoa-600); font-size: 12px; font-weight: 600; }.profile-section-heading p { margin: 5px 0 0; color: var(--cocoa-700); font-size: 13.5px; line-height: 1.45; }.avatar-picker > p { margin: 5px 0 0; color: var(--cocoa-700); font-size: 13.5px; line-height: 1.45; }
.profile-section-heading--security { align-items: center; margin-bottom: 0; }.profile-section-heading--security > div { min-width: 0; flex: 1; }
.profile-fields { display: grid; grid-template-columns: 1fr 1fr; gap: 18px; }.profile-fields__wide { grid-column: 1 / -1; }.profile-fields label > span { display: block; margin-bottom: 7px; color: var(--cocoa-800); font-size: 13px; font-weight: 700; }.password-grid label > span { display: block; margin-bottom: 7px; color: var(--cocoa-800); font-size: 13px; font-weight: 700; }.profile-fields label > span small { color: var(--cocoa-600); font-weight: 500; }.character-count { display: block; margin-top: 6px; color: var(--cocoa-600); font-size: 12px; text-align: right; }
.avatar-picker { margin: 0; padding-top: 20px; }.avatar-grid { display: grid; grid-template-columns: repeat(5, minmax(0, 1fr)); gap: 12px; margin-top: 20px; }.avatar-grid input { position: absolute; width: 1px; height: 1px; opacity: 0; }.avatar-grid__label { overflow: hidden; width: 100%; color: var(--cocoa-800); font-size: 11.5px; font-weight: 700; text-align: center; text-overflow: ellipsis; white-space: nowrap; }.avatar-grid label.selected > svg { opacity: 1; transform: scale(1); }
.password-grid { display: grid; grid-template-columns: repeat(3, minmax(0, 1fr)); gap: 16px; margin-top: 22px; }.password-input input { min-width: 0; width: 100%; padding: 0 0 0 13px; border: 0; outline: 0; color: var(--cocoa-900); background: transparent; font-size: 15px; }.password-input button { display: grid; place-items: center; width: 44px; height: 44px; border: 0; border-radius: 9px; color: var(--cocoa-600); background: transparent; cursor: pointer; }
...
.profile-card { min-width: 0; padding: 26px; border: 2px solid var(--cocoa-950); border-radius: 10px; background: var(--cream-50); box-shadow: 4px 4px 0 var(--cocoa-950); }
.profile-overview { display: flex; align-items: center; gap: 24px; overflow: hidden; background: var(--dorito-100); border-color: var(--cocoa-950); }
.profile-overview__avatar { padding: 6px; border-radius: 50%; background: var(--cream-50); box-shadow: 3px 3px 0 var(--cocoa-950); border: 2px solid var(--cocoa-950); }
.profile-section-heading > span { display: grid; place-items: center; width: 42px; height: 42px; flex: none; border-radius: 8px; color: var(--cocoa-950); background: var(--lavender-100, #ece8ff); border: 2px solid var(--cocoa-950); }
.profile-section-heading--security > button { min-height: 44px; padding: 0 14px; border: 2px solid var(--cocoa-950); border-radius: 8px; color: var(--cocoa-950); background: var(--sage-100, #e4f0e3); font-weight: 800; cursor: pointer; box-shadow: 3px 3px 0 var(--cocoa-950); }
.profile-section-heading--security > button:hover { border-color: var(--cocoa-950); background: var(--sage-200, #d4e8d2); }
.profile-section-heading--security > button:focus-visible { border-color: var(--cocoa-950); background: var(--sage-200, #d4e8d2); }
.profile-fields input { width: 100%; border: 2px solid var(--cocoa-900); border-radius: 8px; color: var(--cocoa-900); background: var(--cream-50); font-size: 15px; min-height: 48px; padding: 0 13px; }
.profile-fields textarea { width: 100%; border: 2px solid var(--cocoa-900); border-radius: 8px; color: var(--cocoa-900); background: var(--cream-50); font-size: 15px; padding: 12px 13px; resize: vertical; line-height: 1.5; }
.password-input { width: 100%; border: 2px solid var(--cocoa-900); border-radius: 8px; color: var(--cocoa-900); background: var(--cream-50); font-size: 15px; display: grid; grid-template-columns: minmax(0, 1fr) 44px; align-items: center; min-height: 48px; }
.profile-fields input[readonly] { color: var(--cocoa-600); background: var(--sand-100, #f3e9da); }
.profile-fields :is(input, textarea):focus { border-color: var(--dorito-600); outline: 3px solid var(--dorito-300); outline-offset: 2px; }
.password-input:focus-within { border-color: var(--dorito-600); outline: 3px solid var(--dorito-300); outline-offset: 2px; }
.avatar-picker legend { margin: 0; color: var(--cocoa-950); font: 800 20px/1.2 var(--font-display); margin-left: 8px; padding: 0 10px; border-radius: 7px; background: var(--cream-50); border: 2px solid var(--cocoa-950); }
.avatar-grid label { position: relative; display: flex; flex-direction: column; align-items: center; gap: 9px; min-width: 0; padding: 13px 8px 11px; border: 2px solid var(--cocoa-900); border-radius: 9px; background: var(--sand-50); cursor: pointer; transition: transform .16s ease, box-shadow .16s ease, background-color .16s ease; box-shadow: 3px 3px 0 var(--cocoa-900); }
...
.avatar-grid label > svg { position: absolute; top: 7px; right: 7px; width: 22px; height: 22px; padding: 3px; border-radius: 50%; color: var(--cocoa-950); background: var(--dorito-300); opacity: 0; transform: scale(.8); transition: opacity .18s ease, transform .18s ease; border: 2px solid var(--cocoa-950); }
.password-input button:hover { color: var(--cocoa-950); background: var(--lavender-100, #ece8ff); }
.password-input button:focus-visible { color: var(--cocoa-950); background: var(--lavender-100, #ece8ff); }
.profile-actions { position: sticky; bottom: 14px; z-index: 4; display: flex; justify-content: flex-end; gap: 12px; padding: 12px; border: 2px solid var(--cocoa-950); border-radius: 10px; background: var(--cream-50); box-shadow: 4px 4px 0 var(--cocoa-950); backdrop-filter: none; }
.profile-actions .ghost-button { border: 2px solid var(--cocoa-950); border-radius: 8px; box-shadow: 3px 3px 0 var(--cocoa-950); transition: transform .16s ease, box-shadow .16s ease, background-color .16s ease; color: var(--cocoa-950); background: var(--lavender-100, #ece8ff); }
.profile-actions .primary-button { border: 2px solid var(--cocoa-950); border-radius: 8px; box-shadow: 3px 3px 0 var(--cocoa-950); transition: transform .16s ease, box-shadow .16s ease, background-color .16s ease; background: var(--dorito-500); }
.profile-actions .ghost-button:hover { transform: translate(2px, 2px); box-shadow: 1px 1px 0 var(--cocoa-950); }
.profile-actions .primary-button:hover:not(:disabled) { transform: translate(2px, 2px); box-shadow: 1px 1px 0 var(--cocoa-950); }
...
@media (prefers-reduced-motion: reduce) { .avatar-grid label > svg { transition: none; } .avatar-grid label { transition: none; } .profile-actions .ghost-button { transition: none; } .profile-actions .primary-button { transition: none; }.avatar-grid label:hover { transform: none; }.profile-actions .ghost-button:hover { transform: none; }.profile-actions .primary-button:hover:not(:disabled) { transform: none; } }
</style>
