    ...
    <form class="profile-form" novalidate @submit.prevent="submit">
      ...
      <section class="profile-card" aria-labelledby="personal-data-title">
        ...
        <div class="profile-fields">
          <label for="profile-name"><span>Nombre</span><input id="profile-name" v-model="form.name" name="name" type="text" autocomplete="name" maxlength="120" required /></label>
          ...
          <label class="profile-fields__wide" for="profile-biography"><span>Sobre vos <small>Opcional</small></span><textarea id="profile-biography" v-model="form.biography" name="biography" rows="4" maxlength="500" placeholder="Contá algo que quieras recordar sobre tu proceso…" aria-describedby="profile-biography-count"></textarea><small id="profile-biography-count" class="character-count">{{ biographyCount }}/500</small></label>
        </div>
      </section>
      ...
      <section class="profile-card" aria-labelledby="security-title">
        ...
        <div v-if="passwordChangeEnabled" id="profile-password-fields" class="password-grid">
          <label for="current-password"><span>Contraseña actual</span><div class="password-input"><input id="current-password" v-model="form.currentPassword" name="currentPassword" :type="showCurrentPassword ? 'text' : 'password'" autocomplete="current-password" /><button type="button" :aria-label="showCurrentPassword ? 'Ocultar contraseña actual' : 'Mostrar contraseña actual'" :aria-pressed="showCurrentPassword" @click="showCurrentPassword = !showCurrentPassword"><FontAwesomeIcon :icon="showCurrentPassword ? faEyeSlash : faEye" /></button></div></label>
          <label for="new-password"><span>Nueva contraseña</span><div class="password-input"><input id="new-password" v-model="form.password" name="password" :type="showPassword ? 'text' : 'password'" autocomplete="new-password" minlength="8" placeholder="Mínimo 8 caracteres" /><button type="button" :aria-label="showPassword ? 'Ocultar nueva contraseña' : 'Mostrar nueva contraseña'" :aria-pressed="showPassword" @click="showPassword = !showPassword"><FontAwesomeIcon :icon="showPassword ? faEyeSlash : faEye" /></button></div></label>
          <label for="password-confirmation"><span>Confirmar contraseña</span><div class="password-input"><input id="password-confirmation" v-model="form.passwordConfirmation" name="passwordConfirmation" :type="showConfirmation ? 'text' : 'password'" autocomplete="new-password" minlength="8" /><button type="button" :aria-label="showConfirmation ? 'Ocultar confirmación' : 'Mostrar confirmación'" :aria-pressed="showConfirmation" @click="showConfirmation = !showConfirmation"><FontAwesomeIcon :icon="showConfirmation ? faEyeSlash : faEye" /></button></div></label>
        </div>
      </section>
